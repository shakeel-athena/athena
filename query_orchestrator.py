...
class HybridToolSelector:
    ...
    def __init__(self, llm_client: LLMClient, suricata_enabled: bool = False):
        ...
        self.HIGH_CONFIDENCE_PATTERNS = {
            ...
            "hids_explicit": {
                "patterns": [
                    ...
                    r"\bhost\s+(ids|ips)\b",
                    r"\bendpoint\s+(intrusion|detection)\s+(detection|system|engine)\b",
        ...
        self.KEYWORD_PATTERNS = {
            ...
            "alert_general": {
                "keywords": ["alert", "alerts", "incident", "triggered"],
                "exclude_keywords": [
                    "search", "virustotal", "fim", "syscheck", "pattern", "patterns", "summary",
                    "network", "ids", "ips", "nids", "suricata", "intrusion", "signature",
                    "packet", "traffic", "flow",
                ],
                "tool": "get_wazuh_alerts",
                "secondary": [],
        ...
        if self.suricata_enabled:
            self.KEYWORD_PATTERNS.update({
                ...
                "network_alerts_kw": {
                    "keywords": [
                        "network alert", "network alerts", "network event", "network events",
                        "network detection", "network incident",
                        "ids alert", "ips alert", "nids alert",
                        "intrusion alert", "intrusion detection event",
                        "signature hit", "signature trigger",
                    ],
                    "exclude_keywords": ["wazuh", "agent", "vulnerability", "cve"],
                    "tool": "get_suricata_alerts",
                    "secondary": [],
                    "confidence": 0.80,
                    "description": "Network/IDS/IPS/intrusion keywords -> Suricata alerts"
                },
                "network_traffic_kw": {
                    "keywords": [
                        "network flow", "packet analysis", "packet capture",
                        "traffic spike", "traffic anomaly", "network activity",
                        "network behavior", "egress traffic", "ingress traffic",
                        "connection analysis",
                    ],
                    "exclude_keywords": ["wazuh", "agent", "vulnerability"],
                    "tool": "get_suricata_network_analysis",
                    "secondary": [],
                    "confidence": 0.75,
                    "description": "Network traffic / packet keywords -> Suricata network analysis"
                },
    ...
    async def select_tools(self, query: str, context: Optional[QueryContext] = None) -> ToolPlan:
        ...
        if all_regex_matches:
            ...
            plan = self._apply_post_selection_corrections(plan, entities, query_lower, query)
        ...
        try:
            ...
            # Post-selection corrections (agent-specific vulns, multi-intent)
            validated_plan = self._apply_post_selection_corrections(validated_plan, entities, query_lower, query)
    ...
    async def _resolve_agent_name_to_id(self, name: str) -> Optional[str]:
        ...
        return self._agent_name_cache.get(name_lc)
...
class QueryOrchestrator:
    ...
    def _build_query_header(self, plan, user_query, primary_result, correlated_data):
        ...
        if correlated_data and hasattr(correlated_data, 'correlated_data') and correlated_data.correlated_data:
            ...
            try:
                normalized = self._normalize_tool_payload(primary_result.data)
                count = len(normalized.get("affected_items", []))
                if count > 0:
                    header += f"**Results:** {count} records found\n"
            except Exception:
                pass
