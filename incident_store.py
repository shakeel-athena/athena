...
class IncidentStore:
    ...
    async def update_incident(self, alert_id: str, updates: Dict[str, Any], action_log: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            # `refresh=wait_for` — same fix as create_incident. OpenSearch's
            # default 1s refresh interval means the immediate get_incident()
            # call below can return the STALE document without the new
            # `updates` fields applied. That stale read flows back to the
            # frontend which merges it into local state, so e.g. setting
            # observability_completed=True here would silently not appear in
            # the analyst's UI until the next page refresh — Block button
            # then stays disabled even after enrichment returns malicious.
            # wait_for adds up to ~1s of latency to each action (acceptable
            # for analyst-driven workflows) in exchange for read-after-write
            # consistency.
            if action_log:
                resp = await self.client.post(
                    f"/{self.INDEX_NAME}/_update/{doc_id}?refresh=wait_for",
                    json=script,
                )
            else:
                resp = await self.client.post(
