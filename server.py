...
# AGELIA_API_URL + INTEGRATION_API_KEY are kept for the deprecated _notify_agelia()
# helper during the transition period — will be removed post-cutover.
...
if AGELIA_NOTIFY_ENABLED:
    logger.info(f"  Agelia legacy bridge: ON ({AGELIA_API_URL}) [DEPRECATED]")
...
async def _notify_agelia(action: str, analyst: str, alert_id: str = '',
                         ip: str = '', rule_id: str = '',
                         signature_id: str = '', reason: str = '',
                         conditions: dict = None, classification: str = '') -> None:
    """DEPRECATED — Use _call_agelia() with Keycloak JWT forwarding instead.

    This helper called the Integration Bridge (/api/integrations/pallas/action)
    via X-Integration-Key. All analyst response actions (block, mute, enrich)
    now go through _call_agelia() which forwards the analyst's Keycloak JWT.
    This function is retained only as a fallback for any callers not yet migrated.
    """
    logger.warning(
        "_notify_agelia() is deprecated. action=%s should use _call_agelia() instead.", action
    )
    ...
    try:
        async with httpx.AsyncClient(timeout=4.0) as client:
            ...
            if resp.status_code == 200:
                logger.info("Agelia legacy bridge notified: action=%s result=%s", action,
                            resp.json().get('data', {}).get('mute_rule_id'))
            else:
                logger.warning("Agelia legacy bridge returned %s: %s",
                               resp.status_code, resp.text[:200])
    except Exception as exc:
        logger.warning("Agelia legacy bridge failed (non-critical): %s", exc)
...
async def acknowledge_incident(request: Request):
    ...
    try:
        ...
        # Sync acknowledge state to Agelia PostgreSQL (best-effort)
        try:
            await _call_agelia(
                "POST", f"/api/incidents/{alert_id}/state", request,
                {"status": "acknowledged", "notes": f"Acknowledged by {analyst_name}"}
            )
        except Exception as sync_err:
            logger.warning(f"Agelia incident sync failed (acknowledge {alert_id}): {sync_err}")
...
async def waf_block_ip(request: Request):
    ...
    result, status = await _call_agelia("POST", "/api/firewall/block-from-alert", request, {
        "ip":           ip,
        "alert_id":     alert_id,
        "alert_source": alert_source,
        "analyst":      analyst_name,
        "reason":       f"Blocked from Pallas Attack Narrative by {analyst_name}",
    })
...
async def close_incident(request: Request):
    ...
    # Sync close state to Agelia PostgreSQL (best-effort)
    try:
        await _call_agelia(
            "POST", f"/api/incidents/{alert_id}/state", request,
            {"status": "closed", "notes": resolution_notes}
        )
    except Exception as sync_err:
        logger.warning(f"Agelia incident sync failed (close {alert_id}): {sync_err}")
...
async def dismiss_false_positive(request: Request):
    ...
    # Determine conditions from the existing incident alert metadata
    ...
    # Notify Agelia so the mute rule is created and Wazuh/Suricata sync applied
    if conditions:
