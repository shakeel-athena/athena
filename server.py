...
def _block_path_configured() -> bool:
    ...
    Available via either:
      ...
      - Agelia proxy: AGELIA_BASE_URL set (handles WAF + Network Firewall + audit)
    ...
    return has_aws or bool(AGELIA_BASE_URL)
...
async def _get_mute_summary() -> list:
    ...
    # When AGELIA_BASE_URL is unset (standalone release), short-circuit so the
...
async def _get_blocked_ips_set() -> set:
    ...
    # When AGELIA_BASE_URL is unset (standalone release), return empty set so
...
"""
 ...
 # Add logcollector stats if available formatted as readable summary
        ...
        # Vulnerability Management Tools
            ...
            logger.error(f"   agent_id={agent_id}, severity={severity}, os={os_filter}, limit={limit}")
            try:
                result = await wazuh_client.get_vulnerabilities(
                    agent_id=agent_id, severity=severity, os=os_filter, limit=limit
                )
            except Exception as e:
                if agent_id and _is_agent_not_found(e):
                    return _agent_not_found_response(agent_id)
...
async def get_meta(request: Request):
    ...
    try:
        ...
        meta_response = {
            ...
            "capabilities": {
                "agelia_enabled": bool(AGELIA_BASE_URL),
                "mute_enabled": False,  # RE-ENABLE NEXT RELEASE — flip to bool(AGELIA_BASE_URL)
...
def _normalize_wazuh_alert(alert: dict, idx: int) -> dict:
    ...
    # Use OpenSearch's stable doc _id (preserved by the wrapper as `_doc_id`).
    # Fallback to position-based id only if `_doc_id` is missing — defensive
    # for any code path that bypasses the indexer wrapper. Position-based ids
    # collide across fetches, which is what produced the ghost-status bug.
    doc_id = alert.get("_doc_id")
    alert_id = f"wz_{doc_id}" if doc_id else f"wz_{idx}_{rule.get('id', '')}"
    return {
        "id": alert_id,
        "source": "wazuh",
        "timestamp": ts,
...
def _normalize_suricata_alert(alert: dict, idx: int) -> dict:
    ...
    # Use OpenSearch's stable doc _id (preserved by the wrapper as `_doc_id`).
    # Fallback to position-based id only if `_doc_id` is missing.
    doc_id = alert.get("_doc_id")
    alert_id = f"sr_{doc_id}" if doc_id else f"sr_{idx}_{alert_data.get('signature_id', '')}"
    return {
        "id": alert_id,
        "source": "suricata",
        "timestamp": ts,
