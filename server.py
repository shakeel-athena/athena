...
from wazuh_mcp_server.aws_waf_client import AWSWAFClient
from wazuh_mcp_server.aws_firewall_client import AWSFirewallClient
...
firewall_client = AWSFirewallClient()
logger.info(f"  SOC Workflow: Jira={'ON' if jira_client_instance.is_configured else 'OFF'}, WAF={'ON' if waf_client.is_configured else 'OFF'}, Firewall={'ON' if firewall_client.is_configured else 'OFF'}")
...
if AGELIA_NOTIFY_ENABLED:
    logger.info(f"  Agelia integration: ON ({AGELIA_API_URL})")
else:
    logger.info("  Agelia integration: OFF (set AGELIA_API_URL + INTEGRATION_API_KEY to enable)")


def _alert_matches_agelia_mutes(alert: dict, mute_rules: list) -> bool:
    """Return True if alert matches any active Agelia mute rule (AND logic per rule)."""
    field_map = {
...
async def _notify_agelia(action: str, analyst: str, alert_id: str = '',
                         ip: str = '', rule_id: str = '',
                         signature_id: str = '', reason: str = '') -> None:
    """Fire-and-forget notification to Agelia after a Pallas response action.
    Never raises — Pallas incident flow must not depend on Agelia availability."""
    ...
    try:
        async with httpx.AsyncClient(timeout=4.0) as client:
            resp = await client.post(
                f"{AGELIA_API_URL}/api/integrations/pallas/action",
                json=payload,
                headers={'X-Integration-Key': AGELIA_API_KEY},
            )
            if resp.status_code == 200:
                logger.info("Agelia notified: action=%s result=%s", action,
                            resp.json().get('data', {}).get('mute_rule_id'))
            else:
                logger.warning("Agelia notification returned %s: %s",
                               resp.status_code, resp.text[:200])
    except Exception as exc:
        logger.warning("Agelia notification failed (non-critical): %s", exc)
...
async def generate_narrative(request: Request):
    ...
    # ── Filter out alerts already muted in Agelia ─────────────────────
    ...
    if AGELIA_NOTIFY_ENABLED:
        try:
            ...
            if resp.status_code == 200:
                mute_rules = resp.json().get('data', {}).get('rules', [])
                if mute_rules:
                    filtered = []
                    for alert in unified_alerts:
                        if not _alert_matches_agelia_mutes(alert, mute_rules):
                            filtered.append(alert)
                    agelia_muted_count = len(unified_alerts) - len(filtered)
                    unified_alerts = filtered
        except Exception as exc:
            logger.warning("Agelia mute filter fetch failed (showing all alerts): %s", exc)
...
async def waf_block_ip(request: Request):
    """Block an attacker IP on AWS WAF."""
    if not waf_client.is_configured:
        return JSONResponse(
            content={"error": "AWS WAF not configured. Set WAF_IP_SET_NAME, WAF_IP_SET_ID in environment. Ensure boto3 is installed."},
            status_code=503,
        )
    ...
    # Execute WAF block (synchronous boto3 call)
    import asyncio
    loop = asyncio.get_running_loop()
    result = await loop.run_in_executor(None, waf_client.block_ip, ip)
    ...
    if "error" in result:
        ...
        await incident_store.update_incident(
            alert_id,
            {"waf_error": result["error"]},
            action_log={
                "action": "waf_block_failed",
                "timestamp": now,
                "actor": analyst_name,
                "details": f"WAF block failed for {ip}: {result['error']} (by {analyst_name})",
            },
        )
        return JSONResponse(content=result, status_code=502)
    ...
    await incident_store.update_incident(
        ...
        action_log={
            ...
            "timestamp": now,
            "actor": analyst_name,
            "details": f"Blocked {ip} on AWS WAF ({waf_client.ip_set_name}) by {analyst_name}",
        },
    )

    # Add Jira comment for WAF block audit trail
    if existing and existing.get("jira_ticket_id") and jira_client_instance.is_configured:
        await jira_client_instance.add_comment(
            ...
            f"WAF IP block executed by {analyst_name}: {ip} blocked on AWS WAF ({waf_client.ip_set_name})",
        )

    # Notify Agelia — auto-creates mute rule for this IP
    await _notify_agelia('block_waf', analyst=analyst_name, alert_id=alert_id, ip=ip,
                         reason=f'Blocked on AWS WAF by Pallas analyst {analyst_name}')
...
async def firewall_block_ip(request: Request):
    """Block an attacker IP on AWS Network Firewall."""
    if not firewall_client.is_configured:
        return JSONResponse(
            content={"error": "AWS Network Firewall not configured. Set NETWORK_FW_RULE_GROUP_ARN, NETWORK_FW_RULE_GROUP_NAME in environment."},
            status_code=503,
        )
    ...
    # Execute firewall block (synchronous boto3 call)
    import asyncio
    loop = asyncio.get_running_loop()
    result = await loop.run_in_executor(None, firewall_client.block_ip, ip)
    ...
    if "error" in result:
        ...
        await incident_store.update_incident(
            ...
            action_log={
                "action": "fw_block_failed",
                "timestamp": now,
                "actor": analyst_name,
                "details": f"Firewall block failed for {ip}: {result['error']} (by {analyst_name})",
            },
        )
        return JSONResponse(content=result, status_code=502)
    ...
    await incident_store.update_incident(
        ...
        action_log={
            ...
            "timestamp": now,
            "actor": analyst_name,
            "details": f"Blocked {ip} on Network Firewall ({firewall_client.rule_group_name}) by {analyst_name}",
        },
    )

    # Add Jira comment for firewall block audit trail
    if existing and existing.get("jira_ticket_id") and jira_client_instance.is_configured:
        await jira_client_instance.add_comment(
            existing["jira_ticket_id"],
            f"Network Firewall block executed by {analyst_name}: {ip} blocked on AWS Network Firewall ({firewall_client.rule_group_name})",
        )

    # Notify Agelia — auto-creates mute rule for this IP
    await _notify_agelia('block_firewall', analyst=analyst_name, alert_id=alert_id, ip=ip,
                         reason=f'Blocked on Network Firewall by Pallas analyst {analyst_name}')
...
async def get_integration_status(request: Request):
    ...
    return JSONResponse(content={
        ...
        "waf": waf_client.is_configured,
        "firewall": firewall_client.is_configured,
...
async def dismiss_false_positive(request: Request):
    """Dismiss an alert as a false positive. No Jira ticket created."""
    try:
        body = await request.json()
    except Exception:
        return JSONResponse(content={"error": "Invalid JSON"}, status_code=400)
    ...
    # Check if incident exists (must be acknowledged first)
    existing = await incident_store.get_incident(alert_id)
    if not existing:
        return JSONResponse(content={"error": "Incident not found. Acknowledge the alert first."}, status_code=400)

    # FP is allowed after ticket creation — ticket stays in Jira for reference
    ...
    # Notify Agelia — auto-creates mute rule for this rule_id or signature_id
    rule_id_val = existing.get('rule_id', '') or existing.get('wazuh_rule_id', '')
    sig_id_val  = existing.get('signature_id', '') or existing.get('suricata_signature_id', '')
    await _notify_agelia('dismiss_fp', analyst=analyst_name, alert_id=alert_id,
                         rule_id=str(rule_id_val), signature_id=str(sig_id_val),
                         reason=reason or 'Dismissed as false positive in Pallas')
...
async def startup_event():
    ...
    try:
        ...
        logger.error(f" Query Orchestrator initialization failed: {e}")
