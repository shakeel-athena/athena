...
_Generated by Pallas AI Security Platform_"""
    ...
    async def create_ticket(self, alert: Dict[str, Any], narrative: Optional[str] = None, incident_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        # JSM form enforces these fields as required. Option-field labels
        # below match the actual JSM-configured options for the SECOPS project
        # (verified via createmeta API). Override via env if a value gets
        # rejected after a JSM admin migration.
        #
        # Confirmed allowedValues from createmeta:
        #   Severity      : Sev-1 / Sev-2 / Sev-3 / Sev-4
        #   Asset Type    : Cloud Resource / Server | User Endpoint | GitHub Repo | Web Application
        #   Affected Env  : TEST | PROD - Athena | PROD - NBS | PROD - Dakine | PROD - Demo
        alert_source = (alert.get("source") or "unknown").lower()
        rule_groups = alert.get("rule_groups") or []
        rule_groups_text = " ".join(str(g).lower() for g in rule_groups if g)
        ...
        # 10594 — Asset Type (option) — smart routing by alert context
        # GitHub-related rules → GitHub Repo
        # Suricata + HTTP context → Web Application
        # Suricata other / Wazuh server alerts → Cloud Resource / Server
        # Wazuh endpoint alerts → User Endpoint (override via JIRA_ASSET_TYPE_WAZUH)
        if "github" in rule_groups_text or "git_repository" in rule_groups_text:
            asset_type = os.getenv("JIRA_ASSET_TYPE_GITHUB", "GitHub Repo")
        elif alert_source == "suricata" and (
            alert.get("http_url") or alert.get("http_hostname") or alert.get("http_method")
        ):
            asset_type = os.getenv("JIRA_ASSET_TYPE_WEB", "Web Application")
        elif alert_source == "wazuh":
            asset_type = os.getenv("JIRA_ASSET_TYPE_WAZUH", "Cloud Resource / Server")
        elif alert_source == "suricata":
            asset_type = os.getenv("JIRA_ASSET_TYPE_SURICATA", "Cloud Resource / Server")
        else:
            asset_type = "Cloud Resource / Server"
        payload["fields"]["customfield_10594"] = {"value": asset_type}

        # 10405 — Severity (option) — map UPPERCASE severity to JSM "Sev-N" labels
        severity_label_map = {
            "CRITICAL": "Sev-1",
            "HIGH":     "Sev-2",
            "MEDIUM":   "Sev-3",
        ...
        payload["fields"]["customfield_10405"] = {
            "value": severity_label_map.get(severity, "Sev-3")
        }
        ...
        # Default tenant is Athena prod; switch via JIRA_AFFECTED_ENV for other
        # tenants (PROD - NBS, PROD - Dakine, PROD - Demo, TEST).
        affected_env = os.getenv("JIRA_AFFECTED_ENV", "PROD - Athena")
